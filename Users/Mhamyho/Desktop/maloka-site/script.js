...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 4. LIVE COUNTDOWN TO 11/8 ---
    function updateCountdown() {
        const now = new Date();
        // Target Nov 8th or Aug 11th (User specified 11/8)
        let targetYear = now.getFullYear();
        let bdayTarget = new Date(targetYear, 10, 8); // Nov 8 (Month 10 index)
        
        // If date passed this year, target next year
        if (now > bdayTarget && (now.getMonth() !== 10 || now.getDate() !== 8)) {
            bdayTarget = new Date(targetYear + 1, 10, 8);
        }
        ...
        class BdayParticle {
            ...
            draw() {
                ...
                if (this.isHeart) {
                    ...
                    // Floating balloon / glowing circle
                    ctx.beginPath();
